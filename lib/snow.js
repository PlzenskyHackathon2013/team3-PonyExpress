...
function animation() {  // animation function
    ...
    for (i = 0; i < cv.length; ++ i) {  // iterate for every dot
        ...
        miX = mouseX - ($("#dot"+i).width() / 2);
        miY = mouseY - ($("#dot"+i).height() / 2);

        absX = Math.abs(px[i] - miX);
        absY = Math.abs(py[i] - miY);
        ...
        if(absY  < mouseDis){
            ...
            }else if(py[i] < (mouseY + $("#dot"+i).height())){
